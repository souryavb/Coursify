import streamlit as st
import requests
from streamlit_extras.app_logo import add_logo
from modules.nav import SideBarLinks
import pandas as pd

API_URL = "http://web-api:4000/d/data/professor"

# Back button
if st.button("← Back"):
    st.switch_page("pages/40_data_A_Home.py")

response = requests.get(API_URL)
response.raise_for_status()
data = response.json()

if data:
    st.header("Professor Ratings 👨‍🏫")
    
    # Loop through each professor
    for prof in data: 
        
        # Display data in columns for each professor
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric(f"Professor Name", prof['last_name'])
        
        with col2:
            st.metric("Average Rating", f"{float(prof['average_rating']):.2f}")
        
        with col3:
            st.metric("Total Ratings", prof['total_ratings'])
        
        with col4:
            if st.button("View all ratings", key=f"prof_{prof['profID']}"):
                st.session_state['selected_prof_id'] = prof['profID']
                st.switch_page("pages/431_specific_prof.py")
        
        st.divider()  

else:
    st.warning("No professor data available")